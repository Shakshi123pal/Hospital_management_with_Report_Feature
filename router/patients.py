...
async def get_patient_byid(patient_id: int, db: Session = Depends(get_db)):
    req_post = db.query(model.Patients)\
        .options(joinedload(model.Patients.doctor))\
        .filter(model.Patients.id==patient_id).first()
    if not req_post:
...
async def edit_patient_details(id: int, patient: ui_patient, db: Session = Depends(get_db),
    ...
    req_post = db.query(model.Patients).filter(model.Patients.id ==id).first()
    if not req_post:
        raise get_notfound_exception()
...
async def delete_patient_details(id: int, db: Session = Depends(get_db), adm: dict = Depends(get_current_user)):
    ...
    if not req_post:
        raise get_notfound_exception()
    db.query(model.Patients).filter(model.Patients.id==id).delete()
    db.query(model.link).filter(model.link.patient_id==id).delete()
...
async def assign_patients(patient_id: int, doc_id: int, db: Session = Depends(get_db),
    ...
    if not adm:
        return get_user_exception()
    req_patient = db.query(model.Patients).filter(model.Patients.id==patient_id).first()
    if not req_patient:
        raise get_notfound_exception()
